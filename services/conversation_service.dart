...
class ConversationService {
  ...
  Future<Conversation> createConversation({
    ...
    try {
      ...
      // Conversation 모델에서 toFirestore 시 인코딩을 처리하므로, 여기서는 원본 텍스트 전달
      final newConversation = Conversation(
        id: specificId ?? '', // ID는 Firestore에서 생성되거나 specificId 사용
        userId: userId,
        title: title, // 원본 제목 전달, Conversation.toFirestore에서 인코딩
        lastMessage: '',
      ...
      if (specificId != null && specificId.isNotEmpty) {
        ...
        if (existingDoc.exists) {
          ...
          // existingDoc에서 Conversation 객체 생성 시 자동 디코딩
          return Conversation.fromFirestore(existingDoc);
        } else {
          // 존재하지 않는 경우 새로 생성
          docRef = _firestore.collection('conversations').doc(specificId);
          await docRef.set(newConversation.toFirestore()); // Conversation.toFirestore에서 인코딩
        }
      } else {
        // 새 문서 ID 자동 생성
        docRef = await _firestore
            .collection('conversations')
            .add(newConversation.toFirestore()); // Conversation.toFirestore에서 인코딩
      }
